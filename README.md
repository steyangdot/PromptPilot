...
> **Measured (corrected 2026-07-01):** on **Codex**, bounding the ever-growing native transcript feeds the model **~1.3–2.4× fewer total tokens** at end-state parity — ~1.34× on short in-window chains (chain_auth N=5), rising to **~2.4× on long chains that cross codex's compaction threshold** (chain_long N=3: 12.8M vs 30.8M gross/run). **Uncached (full-price) tokens are parity-to-worse for bounding** — a warm native thread re-reads its history at ~90%-cached rates, and those cheap cache reads are exactly what bounding forgoes — so the honest headline is total tokens, never uncached. On **Claude Code**, native `--resume` already caches history cheaply, so PromptPilot keeps it and wins on the **rewrite** instead — **~1.25× fewer full-price tokens** than the raw prompt + native resume, at parity. The SLM control layer itself is **~0.2%** of the input footprint. These are a **CLI / automation** story — the win compounds across multi-turn *programmatic* runs (agent chains, headless `exec` / `--resume` loops, CI/batch) where the native transcript grows every turn, so it's far smaller on a one-off interactive prompt. ⚠️ An earlier headline here (**~4.2× total / ~2× uncached**) was inflated by a token-accounting artifact — codex reports *thread-cumulative* usage on resumed threads and our harness double-counted it; the full correction is documented in [Thread-Cumulative Usage Correction](docs/THREAD_CUMULATIVE_USAGE_CORRECTION.md). See [Benchmarks](docs/BENCHMARKS.md) and [Hybrid Mode](docs/HYBRID_MODE.md).
