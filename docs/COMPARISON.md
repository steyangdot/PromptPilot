...
| Native session (`claude --resume`, `codex resume` / headless `codex exec resume`) | Carry the full conversation transcript across turns | PromptPilot's `withSession` carries a **bounded** window of one-line intent summaries — flat per-turn cost vs the native transcript that grows every turn (measured 465k→2.36M over 5 codex turns via headless `codex exec resume`). See [Session Memory](https://github.com/steyangdot/PromptPilot/wiki/Session-Memory) |
