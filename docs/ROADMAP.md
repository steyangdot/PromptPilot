...
- **v2 control plane** — JSON `ExecutionSpec` with `route ∈ {clarify, answer, passthrough, act}` plus `intent`, `scope`, `target_files`, `risk`, `memory_record`, on all three backends (`slm-anthropic-v2` / `slm-openai-v2` / `slm-subscription-v2`) and now the auto-detect default for every auth (API key or Max OAuth / ChatGPT). v1 prose envelope kept as fallback parser. ([prpt/core/spec.py](https://github.com/steyangdot/PromptPilot/blob/main/prpt/core/spec.py), [prpt/normalizers/slm_anthropic_v2.py](https://github.com/steyangdot/PromptPilot/blob/main/prpt/normalizers/slm_anthropic_v2.py), [prpt/normalizers/slm_openai_v2.py](https://github.com/steyangdot/PromptPilot/blob/main/prpt/normalizers/slm_openai_v2.py), [prpt/normalizers/slm_subscription_v2.py](https://github.com/steyangdot/PromptPilot/blob/main/prpt/normalizers/slm_subscription_v2.py))
...
- **Session memory with referential gate** — recent turns prepended; `--gate-session` classifier skips history when prompt is self-contained. Default-on; the bounded session is a tool-dependent *cost* optimization (large win on codex, use native `--resume` on claude). (An earlier "+60% claude success" figure was a cached-read artifact and did not reproduce in the clean chain_auth replication.)
