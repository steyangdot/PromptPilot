...
- **Hybrid auth pattern** — API-key SLM + subscription LLM. SLM-layer marginal API spend (~$0.0085 on chain5) vs ~$38 of agent work at per-token API rates — a *marginal* gap only; the subscription side carries a fixed monthly fee + finite quota, so it's not free work. ([Hybrid Mode](https://github.com/steyangdot/PromptPilot/wiki/Hybrid-Mode), [Benchmarks](https://github.com/steyangdot/PromptPilot/wiki/Benchmarks))
