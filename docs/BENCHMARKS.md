...
|---|---|---|---|
| chain5 codex hybrid | API-key Haiku SLM + ChatGPT-subscription codex LLM | Cost structure: metered API vs flat-subscription + SLM | **Same 15-turn chain: ~$38 if paid via metered per-token API, vs a flat subscription fee + ~$0.0085 of API for the SLM layer. The harness converts a usage-scaling agent bill into a predictable monthly cost.** Bounded by subscription quota — see caveat. |
...
- **The win is the cost-structure shift, not a "free money" multiplier.** The harness lets a daily coding-agent user pay a flat subscription fee (already bought, for interactive use) + cents of API, instead of a metered per-token bill that climbs with every run. The ~$38-vs-$0.0085 gap is one workload's marginal-API-vs-metered-API figure. The boundary: subscription quota is finite — within it, additional agent runs are near-free; beyond it (sustained CI/batch automation) it runs out (we hit the ChatGPT usage limit mid-experiment, May 2026), so use the API path for high-volume work. See [Hybrid Mode](https://github.com/steyangdot/PromptPilot/wiki/Hybrid-Mode).
