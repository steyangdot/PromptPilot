...
|---|---|---|---|
| chain5 codex hybrid | API-key gpt-5.4-nano SLM + ChatGPT-subscription codex LLM | Token footprint by layer (15-turn chain) | **SLM control layer ~24k input tokens directs ~12.66M agent input tokens — ~0.2% overhead shapes 99.8% of the work.** Hybrid routes the 0.2% to cheap metered API, the rest to a flat-fee subscription. (≈ $0.0085 API + ~$38-at-list-rates of agent tokens, but tokens are the measured fact — see caveat.) |
